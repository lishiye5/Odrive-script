...
board_parameter = {"brake_res": 0,  # 耗散电阻值(根据实际接入的功率电阻值输入，不接为0)
                   ...
                   "over_volt_level": 30.0,  # 过压保护阈值[抓狂]
...
# 控制器基本参数
controller_base_parameter = {"vel_lim": 30,  # 速度限制（[turn/s）
                             "vel_gain": 0.02,  # 速度环增益
                             "pos_gain": 20,  # 位置环增益
                             "vel_integrator": 1,  # 速度环积分
                             }
# 控制模式
...
# 串口控制参数
uart_parameter = {
    "baudrate": 115200,      # 常用 115200, 如果通信量大可设为 460800
    "enable": True,          # 是否开启串口
    "watchdog_timeout": 0.5  # 串口看门狗超时时间(秒)，防止通信中断飞车
}
...
def set_uart_param(odrv, uart_param):
    print("正在强制关闭看门狗并设置串口...")
    # 强制关闭看门狗
    odrv.axis0.config.enable_watchdog = False
    # 【新增】立即清除可能存在的看门狗错误状态，否则无法进行后续校准
    odrv.axis0.error = 0 
    
    odrv.config.uart_baudrate = uart_param["baudrate"]
    odrv.config.enable_uart = uart_param["enable"]
...
if __name__ == '__main__':
    ...
    if set_flag.upper() == "Y":
        ...
        # --- 串口设置 ---
        set_uart_param(odrv0, uart_parameter)
